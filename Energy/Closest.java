...
class State5 {
    ...
    public State5(int cloudletId, int vmId) {
...
class Qlulu {
    ...
    private Map<State5, Map<Action5, Double>> qTable;
    ...
    public Qlulu(double alpha, double gamma, double epsilon) {
        this.alpha = alpha;
        this.gamma = gamma;
 ...
 // Method to update Q-value based on Q-learning update rule
    public void updateQValue1(State5 currentState, Action5 selectedAction, State5 newState, double reward) {
    ...
    public void updateQValue(State5 currentState, Action5 selectedAction, State5 newState, Action5 newAction, double reward) {
        double oldQValue = qTable.getOrDefault(currentState, new HashMap<>()).getOrDefault(selectedAction, 0.0);
    ...
    private double getMaxQValue(State5 state) {
        Map<Action5, Double> actionValues = qTable.getOrDefault(state, new HashMap<>());
    ...
    public Map<State5, Map<Action5, Double>> getQTable() {
        return qTable;
    }
}

public class EnergySARSAQ {
    private static final int CLOUDLET_N = 1;
    ...
    public static void main(String[] args) throws Exception {
        ...
        Qlulu sarsaProcessor = new Qlulu(0.1, 0.9, 0.1);

        for (int i = 0; i < 1000; i++) {
            for (Cloudlet cloudlet : cloudletList) {
                State5 currentState = getStateRepresentation(cloudlet, vmList);
                Action5 selectedAction = sarsaProcessor.selectAction(currentState);
                executeAction(selectedAction, cloudlet, vmList);
                State5 newState = getStateRepresentation(cloudlet, vmList);
                Action5 newAction = sarsaProcessor.selectAction(newState);
    ...
    private static void printQTable(Qlulu sarsaProcessor) {
        Map<State5, Map<Action5, Double>> qTable = sarsaProcessor.getQTable();
        System.out.println("Q-table:");
        for (State5 state : qTable.keySet()) {
    ...
    private static State5 getStateRepresentation(Cloudlet cloudlet, List<PowerVm> vmList) {
        int cloudletId = cloudlet.getCloudletId();
        int vmId = cloudlet.getVmId();
        return new State5(cloudletId, vmId);
    }

    private static double calculateReward(State5 currentState, State5 newState) {
    ...
    private static double getCompletionTimeOfState(State5 state) {
        // Placeholder: Assume completion time is a random value between 0 and 1000
        return Math.random() * 1000;
    }

    private static void executeAction(Action5 action, Cloudlet cloudlet, List<PowerVm> vmList) {
