...
class State2 {
    ...
    public State2(int cloudletId, int vmId) {
...
class QLearningProcessor2 {
    private double alpha; // Learning rate
    private double gamma; // Discount factor
    private double epsilon; // Epsilon-greedy exploration factor
    private Map<State2, Map<Action2, Double>> qTable;

    public QLearningProcessor2(double alpha, double gamma, double epsilon) {
        this.alpha = alpha;
        this.gamma = gamma;
...
//    public void updateQValue(State2 currentState, Action2 selectedAction, State2 newState, double reward) {
...
//    }
    ...
    public void updateQValueSARSA(State2 currentState, Action2 selectedAction, State2 newState, Action2 newAction, double reward) {
        double oldQValue = qTable.getOrDefault(currentState, new HashMap<>()).getOrDefault(selectedAction, 0.0);
    ...
    private double getMaxQValue(State2 state) {
        Map<Action2, Double> actionValues = qTable.getOrDefault(state, new HashMap<>());
    ...
    public Map<State2, Map<Action2, Double>> getQTable() {
        return qTable;
    }
}

public class SARSA_WHALE_HYBRID {
    ...
    public static void main(String[] args) throws Exception {
        ...
        QLearningProcessor2 qLearningProcessor = new QLearningProcessor2(0.1, 0.9, 0.1);

        for (int i = 0; i < 3000; i++) {
            for (Cloudlet cloudlet : cloudletList) {
                State2 currentState = getStateRepresentation(cloudlet, vmList);
                Action2 selectedAction = qLearningProcessor.selectAction(currentState);
                executeAction(selectedAction, cloudlet, vmList);
                State2 newState = getStateRepresentation(cloudlet, vmList);
                Action2 newAction=qLearningProcessor.selectAction(newState);
                ...
                qLearningProcessor.updateQValueSARSA(currentState, selectedAction, newState, newAction, reward);
            }

        }

        printQTable(qLearningProcessor);
    ...
    private static void printQTable(QLearningProcessor2 qLearningProcessor) {
        Map<State2, Map<Action2, Double>> qTable = qLearningProcessor.getQTable();
        System.out.println("Q-table:");
        for (State2 state : qTable.keySet()) {
    ...
    private static State2 getStateRepresentation(Cloudlet cloudlet, List<Vm> vmList) {
        int cloudletId = cloudlet.getCloudletId();
        int vmId = cloudlet.getVmId();
        return new State2(cloudletId, vmId);
    }

    private static double calculateReward(State2 currentState, State2 newState) {
    ...
    private static double getCompletionTimeOfState(State2 state) {
        // Placeholder: Assume completion time is a random value between 0 and 1000
        return Math.random() * 1000;
    }

    private static void executeAction(Action2 action, Cloudlet cloudlet, List<Vm> vmList) {
